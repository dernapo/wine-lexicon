...
Creamos palabras que no aportan valor "stop words"
Que luego eliminaremos cuando hagamos el análisis
```{r}

stop_words_espanolas <- rbind(data.frame(word = c("si", "mas", "deja", "nose"), stringsAsFactors = FALSE),
                               data.frame(word = tm::stopwords("spanish"), stringsAsFactors = FALSE))


```

Ajustamos algunos errores gramaticales

```{r}

lexico[, tactiles:= stringr::str_replace_all(string = tactiles,
                pattern = "sensacion",
                replacement = "sensación")]

lexico[, boca:= stringr::str_replace_all(string = boca,
                pattern = "sensacion",
                replacement = "sensación")]
